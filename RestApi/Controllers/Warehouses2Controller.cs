...
public class Warehouses2Controller(IDeliveryService service) : ControllerBase
{
    [HttpPost("")]
    ...
    public async Task<IActionResult> CreateDeliveryProcedurally([FromBody] DeliveryDTO dto)
    {
        try
        {
            var createdDelivery = await service.AddDeliveryProcedurally(dto);
